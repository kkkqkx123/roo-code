...
import { type ModelInfo, ANTHROPIC_DEFAULT_MAX_TOKENS } from "@shared/types"
...
describe("getModelParams", () => {
	...
	describe("Format-specific behavior", () => {
		...
		it("should return correct format for gemini", () => {
			const result = getModelParams({
				...geminiParams,
				settings: {},
				model: baseModel,
		...
		it("should not force maxTokens for gemini format", () => {
			const result = getModelParams({
