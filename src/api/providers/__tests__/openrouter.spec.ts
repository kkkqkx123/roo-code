...
describe("OpenRouterHandler", () => {
	...
	describe("createMessage", () => {
		...
		it("throws exception when createMessage fails", async () => {
			const handler = new OpenRouterHandler(mockOptions)
			...
			const generator = handler.createMessage("test", [])
			await expect(generator.next()).rejects.toThrow("Connection failed")
		})

		it("handles 429 rate limit errors from stream", async () => {
			const handler = new OpenRouterHandler(mockOptions)
			const mockStream = {
				async *[Symbol.asyncIterator]() {
	...
	describe("completePrompt", () => {
		...
		it("handles 429 rate limit errors from response", async () => {
			const handler = new OpenRouterHandler(mockOptions)
			const mockError = {
				error: {
