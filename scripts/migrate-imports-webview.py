#!/usr/bin/env python3
"""
批量更新 webview-ui 中的导入语句：将 @roo-code/types 替换为 @shared/types
"""
...
def main():
    ...
    if not webview_dir.exists():
        print(f"错误: webview-ui 目录不存在: {webview_dir}")
        return

    print(f"开始扫描目录: {webview_dir}")
    print("=" * 60)
    ...
    # 遍历所有 TypeScript 和 TSX 文件
    for file_path in webview_dir.rglob("*.ts"):
